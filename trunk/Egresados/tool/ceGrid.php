...
class ceDataGrid
{
	...
	// Default text
	const TXT_RESET = '';
	const TXT_NORESULTS = 'No existen registros, con ese criterio de busqueda!';

	/**
	* Constructor
	*
	* @param ceMySQLAdap $_db  MySQL Adapter class
	* @param string $image_path The path to datagrid images
	*/
	public function __construct(ceMySQLAdap $_db, $image_path = '')
	{
		$this->_db = $_db;
	...
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{
			case self::STDCTRL_EDIT:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_edit . '" alt="Modificar" title="Modificar" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_DELETE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="Eliminar" title="Eliminar" class="tbl-control-image"></a>';
				break;
			default:
	...
	* @param string $image_path The location of the image if type is CUSCTRL_IMAGE cech 2012
	*/
	...
	private function buildHeader()
	{
		...
		foreach ($headers as $t)
		{
									...
									)))
			{
				...
				echo '<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilter(\'' . $t . '\')">Buscar</a></div>';
			}
